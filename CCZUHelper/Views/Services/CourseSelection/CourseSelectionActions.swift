...
extension CourseSelectionView {
    func loadCourses() async {
        ...
        do {
            let items = try await settings.performJwqywxOperation { app in
                ...
                return courses.map { CourseSelectionItem(raw: $0) }
            }
    ...
    func loadGeneralCourses() async {
        ...
        do {
            let result = try await settings.performJwqywxOperation { app in
                let basicInfo = try await app.getStudentBasicInfo()
                guard let info = basicInfo.message.first else {
                    throw CCZUError.missingData(NSLocalizedString("error.missing_student_info", comment: "无法获取学生基本信息"))
                }
            ...
            await MainActor.run {
                availableGeneralCourses = result.0
                selectedGeneralCourseIds = result.1
                isGeneralLoading = false
                print("[DEBUG] availableGeneralCourses count: \(result.0.count)")
                print("[DEBUG] selectedGeneralCourseIds: \(result.1)")
                let selectedDetails = result.0.filter { result.1.contains($0.courseSerial) }.map { "\($0.courseSerial): \($0.raw.courseName)" }
    ...
    func submitSelection() async {
        ...
        do {
            try await settings.performJwqywxOperation { app in
